/**
 * generate-data.js
 * Extraction complète Webflow → data.json
...
const H = { Authorization: `Bearer ${TOKEN}`, 'accept-version': '1.0.0' };
...
async function fetchAll(collectionId) {
  ...
  while (true) {
    ...
    if (items.length >= (d.pagination?.total ?? 0)) break;
    offset += 100;
  }
  return items;
}

// ─── Formatage prix en virgule (ex: 257.22 → "257,22") ───────────────────
...
async function main() {
  ...
  // Indexer les SKUs par product ID
  const skuByProduct = {};
  for (const sku of rawSkus) {
    const fd = sku.fieldData;
    const pid = fd.product;
    if (!pid) continue;
    if (!skuByProduct[pid]) skuByProduct[pid] = [];
    skuByProduct[pid].push(sku);
  }

  // ─── Construire le tableau produits plat (compatible Fuse.js) ─────────
  const products = [];
  const slugIndex = {};
  ...
  for (const p of rawProducts) {
    ...
    // Premier SKU publié pour prix + image
    ...
    // Correction prix aberrant (ex: DomeCam 5MP 2.8mm Blanche = 756700 centimes)
    if (priceRaw > 100000) {
      console.warn(`⚠ Prix suspect pour "${fd.name}": ${priceRaw} centimes → corrigé à ${priceRaw / 100}`);
      priceRaw = priceRaw / 100;
    }

    const slug = fd.slug || '';
    const brand = FABS[fd.fabricants] || '';
    const envOpt = fd.environnement;
    const env = ENVS[envOpt] || '';
    ...
    const product = {
      ...
      couleur: (fd.couleur === 'c4ace7b5c9fa4b41753e2a21972d9f72' ? 'Blanc' :
                fd.couleur === '192fa2d4339dc5af00fe9a51e72bea99' ? 'Noir' : ''),
      environnement: env,
      iacamera: fd['intelligence-artificielle-camera'] || '',
      micro: (fd['micro-integre-2'] === 'b171d95294a5f81060c177713e8b0183' ? 'Oui' :
      ...
      categorie1: '',
      categorie2: '',
      // ── Champs supplémentaires planner ──
      slug,
      serie: fd.serie || '',
  ...
  for (const [key, slug] of Object.entries(PLANNER_MAP_SLUGS)) {
    ...
    if (prod) {
      ...
      console.warn(`⚠ Slug non trouvé dans le CMS pour clé planner "${key}": ${slug}`);
    }
  }

  const resolved = Object.keys(plannerMap).length;
  console.log(`✓ Planner map: ${resolved}/${Object.keys(PLANNER_MAP_SLUGS).length} clés résolues`);
  ...
  console.log(`✅ data.json généré — ${size}KB, ${products.length} produits, ${resolved} clés planner`);
}
